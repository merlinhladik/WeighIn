...
def parse_dokume_qr(url: str) -> dict:
    ...
    if len(parts) != 3:
        raise ValueError("Ungueltiges JWT-Format (erwarte 3 Teile).")
...
def run_qr_scanner(cam_index: int = CAM_INDEX):
    cap = cv2.VideoCapture(cam_index, cv2.CAP_DSHOW)
    if not cap.isOpened():
        raise RuntimeError("Kamera nicht geoeffnet")
